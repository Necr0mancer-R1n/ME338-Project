...
def nn_forward_backward(
    ...
    loss = -torch.log(probs[torch.arange(N, device=X.device), y]).mean()
    ...
    ###########################################################################
    dscores = probs.clone()
    dscores[torch.arange(N, device=X.device), y] -= 1
    dscores /= N
...
def nn_train(
    ...
    for param_name in params:
        params[param_name] = best_params[param_name]
...
def find_best_net(
    ...
    def _take_subset(X, y, count):
        if count >= X.shape[0]:
            return X, y
        idx = torch.randperm(X.shape[0], device=X.device)[:count]
        return X[idx], y[idx]

    # Stage 1: search learning rate / hidden size / regularization on a subset,
    # using a middle decay value. This keeps the broad search affordable.
    coarse_decay = learning_rate_decays[len(learning_rate_decays) // 2]
    coarse_candidates = [
        (lr, hs, reg, coarse_decay)
        for lr in learning_rates
        for hs in hidden_sizes
        for reg in regularization_strengths
    ]

    X_train_s1, y_train_s1 = _take_subset(X_train, y_train, 15000)
    X_val_s1, y_val_s1 = _take_subset(X_val, y_val, 3000)

    coarse_rank = []
    for lr, hs, reg, decay in coarse_candidates:
        net = TwoLayerNet(
            input_size, hs, output_size, dtype=dtype, device=device
        )
        stats = net.train(
            X_train_s1,
            y_train_s1,
        ...
        val_acc = (net.predict(X_val_s1) == y_val_s1).float().mean().item()
        coarse_rank.append((val_acc, lr, hs, reg, decay))

    coarse_rank.sort(reverse=True)
    top_stage1 = coarse_rank[:8]

    # Stage 2: expand only the best coarse triplets across all decay values on
    # a larger subset and with more iterations.
    stage2_candidates = []
    seen = set()
    for _, lr, hs, reg, _ in top_stage1:
        for decay in learning_rate_decays:
            cand = (lr, hs, reg, decay)
            if cand not in seen:
                seen.add(cand)
                stage2_candidates.append(cand)

    X_train_s2, y_train_s2 = _take_subset(X_train, y_train, 30000)
    X_val_s2, y_val_s2 = _take_subset(X_val, y_val, 5000)

    stage2_rank = []
    for lr, hs, reg, decay in stage2_candidates:
        net = TwoLayerNet(
            input_size, hs, output_size, dtype=dtype, device=device
        )
        stats = net.train(
        ...
        val_acc = (net.predict(X_val_s2) == y_val_s2).float().mean().item()
        stage2_rank.append((val_acc, lr, hs, reg, decay))

    stage2_rank.sort(reverse=True)
    top_stage2 = stage2_rank[:4]

    # Stage 3: full-data retraining on the strongest candidates.
    for _, lr, hs, reg, decay in top_stage2:
        net = TwoLayerNet(
            input_size, hs, output_size, dtype=dtype, device=device
        )
        stats = net.train(
