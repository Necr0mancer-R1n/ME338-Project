...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        pred_in_channels = stem_channels[-1] if len(stem_channels) > 0 else in_channels
        self.pred_obj = nn.Conv2d(pred_in_channels, num_anchors, kernel_size=1, stride=1, padding=0)
        self.pred_box = nn.Conv2d(pred_in_channels, 4 * num_anchors, kernel_size=1, stride=1, padding=0)

        for conv in (self.pred_obj, self.pred_box):
            torch.nn.init.normal_(conv.weight, mean=0.0, std=0.01)
            torch.nn.init.constant_(conv.bias, 0.0)
        ######################################################################
        #                           END OF YOUR CODE                         #
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            stem_feat = self.stem_rpn(feat)
            obj = self.pred_obj(stem_feat)
            box = self.pred_box(stem_feat)

            object_logits[level_name] = obj.permute(0, 2, 3, 1).contiguous().view(
                obj.shape[0], -1, self.num_anchors
            )
            boxreg_deltas[level_name] = box.permute(0, 2, 3, 1).contiguous().view(
                box.shape[0], -1, 4 * self.num_anchors
            )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            half_w = width / 2
            half_h = height / 2
            x_ctr, y_ctr = locations.unbind(dim=1)
            anchor_boxes.append(
                torch.stack(
                    [x_ctr - half_w, y_ctr - half_h, x_ctr + half_w, y_ctr + half_h],
                    dim=1,
                )
            )
            ##################################################################
...
def rcnn_get_deltas_from_anchors(
    ...
    gx1, gy1, gx2, gy2 = gt_boxes[:, :4].unbind(dim=1)

    anchor_w = (ax2 - ax1).clamp(min=1e-8)
    anchor_h = (ay2 - ay1).clamp(min=1e-8)
    anchor_ctr_x = ax1 + 0.5 * anchor_w
    anchor_ctr_y = ay1 + 0.5 * anchor_h

    gt_w = (gx2 - gx1).clamp(min=1e-8)
    gt_h = (gy2 - gy1).clamp(min=1e-8)
    gt_ctr_x = gx1 + 0.5 * gt_w
    gt_ctr_y = gy1 + 0.5 * gt_h
    ...
    invalid = gt_boxes[:, :4].min(dim=1).values < 0
    deltas[invalid] = -1e8
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        # The prediction network exposes per-location anchor channels for easier
        # inspection in the notebook; flatten anchors here for loss/proposals.
        pred_obj_logits = {
            level_name: logits.reshape(logits.shape[0], -1)
            for level_name, logits in pred_obj_logits.items()
        }
        pred_boxreg_deltas = {
            level_name: deltas.reshape(deltas.shape[0], -1, 4)
            for level_name, deltas in pred_boxreg_deltas.items()
        }
        shape_per_fpn_level = {
        ...
        if self.training:
            ...
            fg_idx, bg_idx = sample_rpn_training(
                matched_gt_boxes, self.batch_size_per_image * num_images, fg_fraction=0.5
            )
            sampled_idx = torch.cat([fg_idx, bg_idx], dim=0)
            ...
            sampled_box_deltas = pred_boxreg_deltas[sampled_idx]

            gt_objectness = (sampled_gt_boxes[:, 4] >= 0).to(sampled_obj_logits.dtype)
            gt_box_deltas = rcnn_get_deltas_from_anchors(sampled_anchors, sampled_gt_boxes)

            loss_obj = F.binary_cross_entropy_with_logits(
                sampled_obj_logits, gt_objectness, reduction="none"
            )
            loss_box = F.l1_loss(sampled_box_deltas, gt_box_deltas, reduction="none")
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                ##############################################################
                level_deltas = level_boxreg_deltas[_batch_idx].reshape(-1, 4)
                level_scores = level_obj_logits[_batch_idx].reshape(-1).sigmoid()

                proposals = rcnn_apply_deltas_to_anchors(
                    level_deltas.clone(), level_anchors
                )
                proposals[:, 0::2] = proposals[:, 0::2].clamp(
                    min=0, max=image_size[0]
                )
                proposals[:, 1::2] = proposals[:, 1::2].clamp(
                    min=0, max=image_size[1]
                )

                keep_topk = min(self.pre_nms_topk, level_scores.numel())
                level_scores, topk_idx = torch.topk(level_scores, k=keep_topk)
                proposals = proposals[topk_idx]

                keep = torchvision.ops.nms(proposals, level_scores, self.nms_thresh)
                keep = keep[: self.post_nms_topk]
                level_proposals_per_image.append(proposals[keep])
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        # `FCOSPredictionNetwork` for this code block.
        cls_pred = []
        prev_channels = backbone.out_channels
        for out_channels in stem_channels:
            conv = nn.Conv2d(prev_channels, out_channels, kernel_size=3, stride=1, padding=1)
            torch.nn.init.normal_(conv.weight, mean=0.0, std=0.01)
            torch.nn.init.constant_(conv.bias, 0.0)
        ...
        linear = nn.Linear(prev_channels * roi_size[0] * roi_size[1], num_classes + 1)
    ...
    def forward(
        ...
        matched_gt_boxes = []
        for level_name in feats_per_fpn_level.keys():
            # Match in the same order used by roi_align above: level first,
            # then image index within the list of proposal tensors.
            for _idx, proposals_per_image in enumerate(proposals_per_fpn_level[level_name]):
        ...
        # Feel free to delete this line: (but keep variable names same)
        fg_idx, bg_idx = sample_rpn_training(
            matched_gt_boxes, self.batch_size_per_image * num_images, fg_fraction=0.25
        )
        sampled_idx = torch.cat([fg_idx, bg_idx], dim=0)
        sampled_logits = pred_cls_logits[sampled_idx]
        sampled_labels = (matched_gt_boxes[sampled_idx, 4] + 1).long()
