...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.pred_cls = nn.Conv2d(pred_in_channels, num_classes, kernel_size=3, stride=1, padding=1)
        self.pred_box = nn.Conv2d(pred_in_channels, 4, kernel_size=3, stride=1, padding=1)
        self.pred_ctr = nn.Conv2d(pred_in_channels, 1, kernel_size=3, stride=1, padding=1)

        for conv in (self.pred_cls, self.pred_box, self.pred_ctr):
            torch.nn.init.normal_(conv.weight, mean=0.0, std=0.01)
            torch.nn.init.constant_(conv.bias, 0.0)
        ######################################################################
        #                           END OF YOUR CODE                         #
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            ...
            class_logits[level_name] = cls_logits.permute(0, 2, 3, 1).contiguous().view(
                cls_logits.shape[0], -1, cls_logits.shape[1]
            )
            boxreg_deltas[level_name] = box_deltas.permute(0, 2, 3, 1).contiguous().view(
                box_deltas.shape[0], -1, 4
            )
            centerness_logits[level_name] = ctr_logits.permute(0, 2, 3, 1).contiguous().view(
                ctr_logits.shape[0], -1, 1
            )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    is_background = gt_boxes[:, :4].min(dim=1).values < 0
...
def fcos_apply_deltas_to_locations(
    ...
    xc, yc = locations.unbind(dim=1)
    output_boxes = torch.stack([xc - left, yc - top, xc + right, yc + bottom], dim=1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    ##########################################################################
    left, top, right, bottom = deltas.unbind(dim=1)
    valid = deltas.min(dim=1).values >= 0

    lr = torch.minimum(left, right) / torch.maximum(left, right).clamp(min=1e-8)
    tb = torch.minimum(top, bottom) / torch.maximum(top, bottom).clamp(min=1e-8)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for gt_boxes_per_image in gt_boxes:
            matched_gt_boxes.append(
                fcos_match_locations_to_gt(
                    locations_per_fpn_level, self.backbone.fpn_strides, gt_boxes_per_image
                )
            )
        ...
        for matched_boxes_per_image in matched_gt_boxes:
            matched_deltas_per_image = {}
            for level_name, matched_boxes_this_level in matched_boxes_per_image.items():
                matched_deltas_per_image[level_name] = fcos_get_deltas_from_locations(
                    locations_per_fpn_level[level_name],
                    matched_boxes_this_level,
                    self.backbone.fpn_strides[level_name],
                )
            matched_gt_deltas.append(matched_deltas_per_image)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        foreground_mask = matched_gt_boxes[:, :, 4] >= 0
        if foreground_mask.any():
            batch_idx, loc_idx = foreground_mask.nonzero(as_tuple=True)
            class_idx = matched_gt_boxes[batch_idx, loc_idx, 4].long()
            gt_classes[batch_idx, loc_idx, class_idx] = 1
        ...
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits, gt_centerness.clamp(min=0), reduction="none"
        )
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_boxes[:, 0::2] = level_pred_boxes[:, 0::2].clamp(min=0, max=img_w)
            level_pred_boxes[:, 1::2] = level_pred_boxes[:, 1::2].clamp(min=0, max=img_h)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
