...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx = torch.tensor(x, dtype=torch.int64)
    y = torch.zeros(idx.shape[0], idx.max().item() + 1, dtype=torch.float32)
    y[torch.arange(idx.shape[0]), idx] = 1.0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    row_mins = y.argmin(dim=1)
    y[torch.arange(y.shape[0]), row_mins] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M = x.shape[0]
    mu = x.sum(dim=0).reshape(1, -1) / M
    var = ((x - mu) ** 2).sum(dim=0).reshape(1, -1) / (M - 1)
    sigma = var.sqrt()
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    vals = torch.cat(xs)
    group_ids = torch.repeat_interleave(torch.arange(ls.shape[0], device=ls.device), ls)
    sums = torch.zeros(ls.shape[0], dtype=vals.dtype, device=vals.device)
    sums.scatter_add_(0, group_ids, vals)
    y = sums / ls.to(vals.dtype)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    first_rev = torch.full((uniques.shape[0],), x.shape[0], dtype=torch.int64, device=x.device)
    rev_idx = torch.arange(x.shape[0] - 1, -1, -1, device=x.device)
    first_rev.scatter_(0, inverse.flip(0), rev_idx)
    indices = first_rev
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
