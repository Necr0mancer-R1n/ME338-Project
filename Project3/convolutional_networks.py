...
class Conv(object):
    ...
    def backward(dout, cache):
        ...
        dx = dx_padded[:, :, pad:pad + H, pad:pad + W] if pad > 0 else dx_padded
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
        ...
        for n in range(N):
            for c in range(C):
                for i in range(H_out):
                    ...
                    for j in range(W_out):
                        ws = j * stride
                        window = x[n, c, hs:hs + pool_height,
                                   ws:ws + pool_width]
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W1'] = weight_scale * torch.randn(
            num_filters, C, filter_size, filter_size, dtype=dtype,
            device=device)
        self.params['b1'] = torch.zeros(num_filters, dtype=dtype,
                                        device=device)
        pool_H, pool_W = H // 2, W // 2
        self.params['W2'] = weight_scale * torch.randn(
            num_filters * pool_H * pool_W, hidden_dim, dtype=dtype,
            device=device)
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype,
                                        device=device)
        self.params['W3'] = weight_scale * torch.randn(
            hidden_dim, num_classes, dtype=dtype, device=device)
        self.params['b3'] = torch.zeros(num_classes, dtype=dtype,
                                        device=device)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out1, cache1 = Conv_ReLU_Pool.forward(X, W1, b1, conv_param,
                                               pool_param)
        ...
        _, dW1, db1 = Conv_ReLU_Pool.backward(dout1, cache1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        prev_C = C
        cur_H, cur_W = H, W
        for i, num_filter in enumerate(num_filters, start=1):
            if weight_scale == 'kaiming':
                W = kaiming_initializer(prev_C, num_filter, K=3, relu=True,
                                        device=device, dtype=dtype)
            elif weight_initializer is not None:
                W = weight_initializer(prev_C, num_filter, K=3, relu=True,
                                       device=device, dtype=dtype)
            else:
                W = weight_scale * torch.randn(num_filter, prev_C, 3, 3,
                                               dtype=dtype, device=device)
            self.params['W%d' % i] = W
            self.params['b%d' % i] = torch.zeros(num_filter, dtype=dtype,
                                                 device=device)
            if self.batchnorm:
                self.params['gamma%d' % i] = torch.ones(num_filter,
                                                        dtype=dtype,
                                                        device=device)
                self.params['beta%d' % i] = torch.zeros(num_filter,
                                                        dtype=dtype,
                                                        device=device)
            prev_C = num_filter
        ...
        final_dim = prev_C * cur_H * cur_W
        if weight_scale == 'kaiming':
            W = kaiming_initializer(final_dim, num_classes, relu=False,
                                    device=device, dtype=dtype)
        elif weight_initializer is not None:
            W = weight_initializer(final_dim, num_classes, relu=False,
                                   device=device, dtype=dtype)
        else:
            W = weight_scale * torch.randn(final_dim, num_classes,
                                           dtype=dtype, device=device)
        self.params['W%d' % self.num_layers] = W
        self.params['b%d' % self.num_layers] = torch.zeros(num_classes,
                                                          dtype=dtype,
                                                          device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            W = self.params['W%d' % i]
            loss += self.reg * W.pow(2).sum()

        dx, dw, db = Linear.backward(dout, caches[-1])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.batchnorm:
                dx, dw, db, dgamma, dbeta = (
                    Conv_BatchNorm_ReLU_Pool.backward(cache=cache, dout=dx)
                    if (i - 1) in self.max_pools
                    else Conv_BatchNorm_ReLU.backward(dx, cache)
                )
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            running_mean = momentum * running_mean + (1 - momentum) * sample_mean
            running_var = momentum * running_var + (1 - momentum) * sample_var
            cache = ('train', x_hat, gamma, xmu, inv_std, sample_var, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            out = gamma * x_hat + beta
            cache = ('test', x_hat, gamma, running_var, eps)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        mode = cache[0]
        if mode == 'train':
            _, x_hat, gamma, xmu, inv_std, sample_var, eps = cache
            N = dout.shape[0]
            dbeta = dout.sum(dim=0)
            dgamma = (dout * x_hat).sum(dim=0)
            dxhat = dout * gamma
            dvar = (dxhat * xmu * -0.5 * inv_std.pow(3)).sum(dim=0)
            dmu = (dxhat * -inv_std).sum(dim=0) + dvar * (-2.0 * xmu).mean(dim=0)
            dx = dxhat * inv_std + dvar * 2.0 * xmu / N + dmu / N
        elif mode == 'test':
            _, x_hat, gamma, running_var, eps = cache
            dbeta = dout.sum(dim=0)
            dgamma = (dout * x_hat).sum(dim=0)
            dx = dout * gamma / torch.sqrt(running_var + eps)
        else:
            raise ValueError('Invalid batchnorm cache mode "%s"' % mode)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        if mode == 'train':
            ...
            dgamma = (dout * x_hat).sum(dim=0)
            dxhat = dout * gamma
            dx = (inv_std / N) * (N * dxhat - dxhat.sum(dim=0) -
                                  x_hat * (dxhat * x_hat).sum(dim=0))
        elif mode == 'test':
            _, x_hat, gamma, running_var, eps = cache
            dbeta = dout.sum(dim=0)
            dgamma = (dout * x_hat).sum(dim=0)
