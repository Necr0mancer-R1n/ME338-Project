...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hidden, cache_hidden = Linear_ReLU.forward(X, self.params['W1'],
                                                    self.params['b1'])
        scores, cache_scores = Linear.forward(hidden, self.params['W2'],
                                              self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dhidden, grads['W2'], grads['b2'] = Linear.backward(dscores,
                                                            cache_scores)
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dhidden,
                                                           cache_hidden)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params['W%d' % (i + 1)] = (
                weight_scale * torch.randn(layer_dims[i], layer_dims[i + 1],
                                           dtype=dtype, device=device)
            )
            self.params['b%d' % (i + 1)] = torch.zeros(layer_dims[i + 1],
                                                       dtype=dtype,
                                                       device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            out, layer_cache = Linear_ReLU.forward(out,
                                                   self.params['W%d' % i],
                                                   self.params['b%d' % i])
            if self.use_dropout:
        ...
        scores, final_cache = Linear.forward(out,
        ...
        for i in range(1, self.num_layers + 1):
            loss += 0.5 * self.reg * self.params['W%d' % i].pow(2).sum()
        ...
        for i in range(self.num_layers - 1, 0, -1):
            cache = caches[i - 1]
            if self.use_dropout:
                layer_cache, dropout_cache = cache
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=500, weight_scale=1e-2, reg=1e-3,
                        dtype=dtype, device=device)
    #############################################################
...
def rmsprop(w, dw, config=None):
    ...
    # Replace "pass" statement with your code
    cache = config['cache']
    cache = config['decay_rate'] * cache + (1 - config['decay_rate']) * dw.pow(2)
    next_w = w - config['learning_rate'] * dw / (torch.sqrt(cache) +
                                                 config['epsilon'])
    config['cache'] = cache
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def adam(w, dw, config=None):
    ...
    v = config['beta2'] * config['v'] + (1 - config['beta2']) * dw.pow(2)
    m_unbias = m / (1 - config['beta1'] ** t)
    v_unbias = v / (1 - config['beta2'] ** t)
    next_w = w - config['learning_rate'] * m_unbias / (torch.sqrt(v_unbias) +
                                                        config['epsilon'])
    config['m'] = m
    config['v'] = v
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand_like(x) > p).type_as(x) / (1 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
